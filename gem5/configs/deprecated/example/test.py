...
def makeMeshTopology(n_rows, n_cols, n_cpus, n_xcels, system, network, double_l2,
  ...
  for i in range(n_cols * (n_rows - 1), num_routers):
    if l2_idx < len(l2s):
      l2_ext_link = ExtLink(link_id   = link_count,
                            ext_node  = l2s[l2_idx],
                            int_node  = routers[i],
...
# Scratchpad size
parser.add_argument("--spm-size", action="store", type=str, 
  default="4kB", help="Specify the scratchpad memory size")

# number of pending requests allowed by scratchpad
parser.add_argument("--stream-width", type = int, default = 8)
...
if (args.cpu_type == 'DerivO3CPU'):
  ...
  args.stream_width = CPUClass.LQEntries + CPUClass.SQEntries
else:
  if (args.cpu_type != 'IOCPU'):
    print('WARNING: Only support IOCPU (w or w/o vector) or DerivO3Cpu (w/o vector)')
  # CPU class
  CPUClass = IOCPU (
    includeVector = args.vector,
    meshBufferSize = args.mesh_queue_len,
    numROBEntries = 8,
    # remember to set in util.h
    hw_vector_length = args.hw_vlen
...
system = System(cpu = [ CPUClass(cpu_id = i) for i in range(n_cpus) ],
...
system.cpu_clk_domain = SrcClockDomain(clock = args.sys_clock,
                                       voltage_domain =
...
system.ruby = RubySystem(block_size_bytes=args.cacheline_size)
...
for i in range(n_scratchpads):
  sp = Scratchpad(version           = i,
                  ruby_system       = system.ruby,
                  sp_size           = AddrRange(args.spm_size).size(),
                  dram_size         = AddrRange(args.mem_size).size(),
                  num_l2s           = n_l2s,
                  grid_dim_x        = n_cols,
                  grid_dim_y        = n_cols,
                  # might be too big but can solve by having spad not remember
                  # store noacks like it does for prefetch
                  maxNumPendingReqs = args.stream_width,
...
init_network(args, network, InterfaceClass)
...
if (args.vector and args.cpu_type == 'IOCPU'):
  eff_rows = n_rows - 1
  if (double_L2):
    eff_rows = n_rows - 2
